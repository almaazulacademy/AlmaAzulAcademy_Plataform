...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "group flex flex-1 items-center justify-between py-6 text-left text-lg font-medium transition-colors hover:text-lake [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-6 pr-10 leading-7", className)}>{children}</div>
  </AccordionPrimitive.Content>
));
