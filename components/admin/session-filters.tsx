...
import { adminExperienceLabel, experiencesInBase } from "@/lib/admin/base-filter";
import type { AdminBase, AdminExperience, AdminSessionFilters } from "@/lib/admin/types";
import {
...
export function SessionFilters({ filters, experiences, bases = [] }: { filters: AdminSessionFilters; experiences: AdminExperience[]; bases?: AdminBase[] }) {
  ...
  return (
    <form method="get" className="mt-6 rounded-3xl border border-ink/10 bg-white p-5 sm:p-6" aria-label="Filtros de sessões">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <label><span className={labelClass}>Busca</span><input name="busca" defaultValue={filters.query} className={inputClass} placeholder="Buscar por experiência..." /></label>
        ...
        <label><span className={labelClass}>Experiência</span><select name="experiencia" defaultValue={filters.experienceId} className={inputClass}><option value="">Todas</option>{experiencesInBase(experiences, filters.base).map((experience) => <option key={experience.id} value={experience.id}>{adminExperienceLabel(experience)}</option>)}</select></label>
