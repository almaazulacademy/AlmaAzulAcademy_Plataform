  ...
  return (
    <aside className="rounded-4xl bg-ink p-7 text-white sm:p-9 lg:sticky lg:top-32">
      ...
      <div className="mt-8 space-y-4 border-t border-white/15 pt-7 text-sm text-white/75">
        <p className="flex items-start gap-3"><CalendarDays className="mt-0.5 size-4 shrink-0 text-sand" />{formatSessionDate(session.startsAt)}</p>
        <p className="flex items-center gap-3"><Clock3 className="size-4 text-sand" />{formatSessionTime(session.startsAt)} · {session.durationMinutes} minutos</p>
