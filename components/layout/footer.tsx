  ...
  return (
    <footer className="bg-ink text-white">
      <div className="container py-14 sm:py-20">
        <div className="grid gap-12 border-b border-white/15 pb-14 sm:grid-cols-2 md:grid-cols-[1.4fr_0.6fr_0.6fr_0.6fr]">
          <div>
            <Image
          ...
          <div>
            ...
            <div className="flex flex-col gap-3 text-sm text-white/75">
              {bases.map((base) => (
                <Link key={base.slug} href={`/bases/${base.slug}`} className="inline-flex items-center gap-2 transition-colors hover:text-white">
                  {base.name}
                  {base.status !== "ACTIVE" ? (
                    <span className="rounded-full bg-white/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-white/60">Em breve</span>
                  ) : null}
                </Link>
              ))}
              <Link href="/bases" className="transition-colors hover:text-white">Todas as bases</Link>
            </div>
          </div>
          <div>
            <p className="mb-5 text-xs font-semibold uppercase tracking-[0.18em] text-white/45">
