# Mídias oficiais — Alma Azul

Fonte: pasta oficial aprovada no Google Drive (`18mkunV4wjs7sgDhCRPi2H65WAv8c6QcZ`), separada por experiência.

## Uso recomendado

- Hero da Home e da Imersão: `backgrounds/hero-alma-azul-lago.webp`
- Logo escura: `branding/alma-azul-logo-dark.png`
- Logo branca: `branding/alma-azul-logo-white.png`
- Imersão Paranoá: `experiences/imersao-paranoa/`
- Remada Sunset: `experiences/remada-sunset/`
- Remada do Nascer do Sol: `experiences/remada-nascer-do-sol/`
- Remada da Lua Cheia: `experiences/remada-lua-cheia/`
- Base Concha Acústica (landing e card): `bases/concha-acustica/` e vídeos em `public/videos/bases/concha-acustica/`. Originais na pasta "Capsula + Alma Azul" do Drive; aqui só derivados WebP e MP4 H.264 sem áudio. Fotos e vídeo com canoas são registros da Alma Azul no Lago Paranoá, não da nova base.

## Critérios aplicados

- Apenas arquivos das pastas oficiais da respectiva experiência são utilizados.
- WebPs usam nomes minúsculos, sem espaços ou acentos e com capitalização segura para Linux.
- Nenhum rosto foi desfocado, retocado ou recortado artificialmente para ocultação.
- Fotografias com rosto de cliente reconhecível foram substituídas ou removidas antes do lançamento.
- Nenhum original (HEIC, DNG, JPG de câmera) fica em `public/`: tudo aqui é servido publicamente e entra no deploy. Os originais vivem no Google Drive e no histórico Git; `public/` guarda somente os derivados WebP otimizados.
- O inventário e as decisões estão em `docs/prelaunch-image-privacy-audit.md`.
- Auditoria de metadados (Sprint 6.0): nenhum dos 54 WebP/PNG publicados carrega chunk EXIF ou XMP — logo, nenhum carrega GPS.
