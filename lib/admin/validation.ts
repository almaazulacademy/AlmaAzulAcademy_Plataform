...
export const RESERVED_EXPERIENCE_SLUGS = new Set(["admin", "api", "login", "reservar", "pagamento", "acompanhar-reserva", "experiencias", "imersao-paranoa", "bases"]);
...
export function validateAdminExperienceInput(value: unknown): ValidationResult<AdminExperienceInput> {
  ...
  const baseId = String(value.baseId ?? "").trim();
  ...
  if (!isUuid(baseId)) errors.baseId = "Selecione a base da experiência.";
  ...
  // Experiência publicada precisa de capacidade real. Rascunho e "em breve"
  // aceitam 0 = ainda não definida (como as experiências planejadas da Concha).
  const minimumCapacity = status === "PUBLISHED" ? 1 : 0;
  if (!Number.isInteger(defaultCapacity) || defaultCapacity < minimumCapacity || defaultCapacity > 500) {
    errors.defaultCapacity = minimumCapacity ? "Informe uma capacidade entre 1 e 500." : "Informe uma capacidade entre 0 e 500.";
  }
  ...
  return Object.keys(errors).length
    ? { success: false, errors }
    : { success: true, data: { baseId, title, summary, description, durationMinutes, priceCents, defaultCapacity, status, imageUrl, displayOrder, editorialContent: editorial.success ? editorial.data : editorialValue as never } };
}
