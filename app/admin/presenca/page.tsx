...
export default async function AttendancePage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const requested = typeof params.data === "string" ? params.data : "";
  const date = isDateKey(requested) ? requested : today;

  let sessions: Awaited<ReturnType<typeof listAttendanceSessions>> = [];
  let failure: string | null = null;
  try {
    sessions = await listAttendanceSessions(context.profile.userId, date);
  } catch (error) {
    ...
    <div>
      ...
      <nav className="mt-6 flex flex-wrap items-center gap-2" aria-label="Escolher dia">
        {tabs.map((tab) => (
          <Link
            key={tab.label}
            href={`/admin/presenca?data=${tab.key}`}
        ...
        <form action="/admin/presenca" className="flex items-center gap-2">
            ...
            {sessions.map((session) => {
              ...
              return (
                <li key={session.sessionId}>
                  <Link
                    href={`/admin/presenca/${session.sessionId}`}
